                        ...
                        <div class="card-body">
                            ...
                            <form action="{{ route('jadwal-kajian.store') }}" method="POST">
                                ...
                                <div class="mb-4">
                                    ...
                                    @enderror" name="name" value="{{ old('name') }}" id="name" placeholder="Nama Pemohon">
                                </div>
                                <div class="mb-4">
                                    <label for="tema_kajian" class="form-label fs-6">Tema Kajian</label>
                                    <input type="text" class="form-control @error('tema_kajian') is-invalid
                                    @enderror" name="tema_kajian" value="{{ old('tema_kajian') }}" id="tema_kajian"
                                ...
                                <div class="mb-4">
                                    ...
                                    @enderror" name="lokasi" id="lokasi" value="{{ old('lokasi') }}" placeholder="Sebutkan nama masjid atau lokasi kajian..">
                                </div>
                                <div class="mb-4">
                                    <label for="jenis_kajian" class="form-label fs-6">Jenis Waktu Kajian</label>
                                    <select class="form-select @error('jenis_kajian') is-invalid
                                    @enderror" name="jenis_kajian" id="jenis_kajian" value="{{ old('jenis-kajian') }}">
                                        <option value="">Waktu Kajian</option>
                                        @foreach ($jenisKajians as $jenisKajian)
                                        <option value="{{ $jenisKajian->id }}" {{ old('jenis_kajian') == $jenisKajian->id ? 'selected' : '' }}>
                                            {{ $jenisKajian->name }}
                                        </option>
                                        @endforeach
                                    </select>
                                ...
                                <div class="mb-4">
                                    ...
                                    @enderror" id="tgl_kajian" value="{{ old('tgl_kajian') }}" name="tgl_kajian" class="form-control">
                                </div>
                                <div class="mb-6">
                                    <button type="submit" class="btn btn-primary me-2">Simpan</button>
                                    <a href="{{ route('request-kajian.index') }}" class="btn btn-outline-secondary">Kembali</a>
                                </div>
                            </form>
                        </div>
                    </div>
